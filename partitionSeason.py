...
def storeTNO(rfile, numrow, numcol, lookAhead):#, seasonBounds, season):
   ...
   #sometimes the names are different
    ...
    for y in range(size):
        ...
        i,j = insertDetection(det.fakeid, det.ra, det.dec, 
                        minRA, maxRA, minDec, maxDec, stepRA, stepDec)
        if(i>-1 and j>-1):
            arr[int(i)][int(j)].add(det)
...
def main():
    ...
    args.add_argument('lookAhead', nargs='?', default=50, help='number of nights to look ahead')
    args.add_argument('numCols', nargs='?', default=60, help='number of columns to partition season')
    args.add_argument('numRows', nargs='?', default=60, help='number of rows to partition season')
    args = args.parse_args()
    regionList = storeTNO(args.seasonFile[0], float(args.numRows), float(args.numCols), 
                        int(args.lookAhead))
    print('\n')
    #maintain a consistent naming convention
    csvFile = args.seasonFile[0].split('+')[-1].split('/')[-1].split('.')[0]
    writeRegion('regions+' + csvFile + '.txt', regionList, 
                float(args.numCols), float(args.numRows))
